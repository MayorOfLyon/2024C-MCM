...
available_state = [0,1,2,3,4,5,6,7,8]
actions = [0,1]

max_children_number = len(available_state)
max_round = 100
random.seed(1)

prob_action = np.loadtxt('probs_matrix.txt')
prob_action_state = np.load('probs_matrix_next.txt.npy')
...
class State(object):
  ...
  def is_terminal(self):
    ...
    return self.current_round_index == max_round

  def compute_reward(self):
    return self.current_value

  def get_next_state_with_random_choice(self):
    random_action = np.random.choice(actions, p = prob_action[self.current_state])
    next_state = State()
    next_state.set_current_round_index(self.current_round_index + 1)
    next_state.set_cumulative_choices(self.cumulative_choices + [actions[random_action]])
    next_state.set_current_action(actions[random_action])
    random_state = np.random.choice(available_state, p = prob_action_state[self.current_state][random_action])
    
    next_state.set_current_state(available_state[random_state])
    if random_state > self.get_current_state() :
      next_value = self.current_value + 1* gamma** (self.get_current_round_index()-1)
    elif random_state < self.get_current_state():
      next_value = self.current_value -1* gamma** (self.get_current_round_index()-1)
    else:
      next_value = self.current_value
    next_state.set_current_value(next_value)
    return next_state

  def __repr__(self):
    return "State: {}, value: {}, round: {}, choices: {}".format(
        self.current_state, self.current_value, self.current_round_index,
        self.cumulative_choices)
